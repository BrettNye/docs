...
```bash
# Disable destructive permissions
curl -X POST 'https://api.example.com/scope-overrides/permissions' \
  -d '{"childScopeId": "scope_prod", "permissionId": "perm_delete", "state": "disabled"}'

curl -X POST 'https://api.example.com/scope-overrides/permissions' \
  -d '{"childScopeId": "scope_prod", "permissionId": "perm_execute", "state": "disabled"}'

curl -X POST 'https://api.example.com/scope-overrides/permissions' \
  -d '{"childScopeId": "scope_prod", "permissionId": "perm_modify_infra", "state": "disabled"}'
```
...
```bash
# Agents can't write to customer data
curl -X POST 'https://api.example.com/scope-overrides/role-permissions' \
  -d '{
    "childScopeId": "scope_customer_data",
    "roleId": "role_agent",
...
```bash
# SOC2 compliance zone
curl -X POST 'https://api.example.com/scope-overrides/permissions' \
  -d '{"childScopeId": "scope_soc2", "permissionId": "perm_export_data", "state": "disabled"}'

# HIPAA compliance zone
curl -X POST 'https://api.example.com/scope-overrides/permissions' \
  -d '{"childScopeId": "scope_hipaa", "permissionId": "perm_share_external", "state": "disabled"}'
```
