...
<Warning>
  **Credentials.** Steps 1–5 are **core control-plane** operations and require a `platform` API key (sent as `x-api-key: brk_...`). The `/evaluate` calls in Step 6 require a tenant-scoped `service` key. The `Authorization: Bearer YOUR_TOKEN` placeholder below stands in for the appropriate key. See [Authentication and Authorization](/api-reference/introduction#authentication-and-authorization).
</Warning>
